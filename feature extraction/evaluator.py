...
def vin_summarize(self):
    ...
    def _summarizeDets():
        ...
        if .5< min_val:
            ...
            val = .5
        stats[1] = _summarize(1, iouThr= val, maxDets=self.params.maxDets[2])
        # stats[2] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
        stats[2] = _summarize(1, iouThr=min_val, maxDets=self.params.maxDets[2])
...
def _evaluate_box_proposals(dataset_predictions, coco_api, thresholds=None, area="all", limit=None):
    ...
    if thresholds is None:
        ...
        thresholds = torch.arange(min_val, max_val + 1e-5, step, dtype=torch.float32)
...
def _evaluate_predictions_on_coco(
    ...
    coco_eval.params.iouThrs = np.linspace(
        min_val, max_val, int(np.round((max_val - min_val) / .05)) + 1, endpoint=True)
