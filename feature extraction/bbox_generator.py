...
def save_h5(dataset, final_features, normalized_bboxes,bboxes, pos_boxes, adj_matrix, test_topk_per_image, pred_classes, times=0, length = 100):
    if dataset == 'mimic':
        filename = './output/mimic_ana_box/bbox_features-25_full.hdf5'
    elif dataset == 'vindr':
...
if __name__ == '__main__':
    ...
    # cfg.MODEL.WEIGHTS = str("results/vindr-online/model_final.pth")
    ...
    # pbar = tqdm(total=len(dloader.dataset))
    # for i, returned_batch in enumerate(dloader):
    #     pbar.update(batch_size)
        if resume:
            if i < continue_i:
                continue
        # dataset_dicts_batch = []
        # data_batch = returned_batch[0]
        # im_batch = returned_batch[1]
        # for j in range(batch_size):
        #     record = {}
        #     for _, key in enumerate(data_batch):
        #         record[key] = data_batch[key][j]
        #     dataset_dicts_batch.append(record)
        # im_list = [im_batch[m] for m in range(batch_size)]
